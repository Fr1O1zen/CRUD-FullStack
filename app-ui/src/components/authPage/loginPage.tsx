...
export default function LoginPage() {
  ...
  const { login, loginError } = useUserLogin({ email, password })
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <div className="hidden lg:block w-1/2 h-screen border-x-amber-300 border-r-2">
        <img src={LoginBackground} alt="Beehive Background" className="w-full h-full object-cover" />
      </div>
      ...
      <div className="w-full lg:w-1/2 flex flex-col justify-center items-center px-6 py-12 bg-gradient-to-b from-slate-900 via-slate-800 to-amber-900/40">
        ...
        <div className="w-full max-w-md">
          ...
          <p className="mt-8 text-center text-white/70">
            Nie masz konta?{" "}
            <Link to="/register" className="text-amber-400 hover:text-amber-300 font-medium underline underline-offset-4 transition-colors">
              Zarejestruj się!
            </Link>
          </p>
        </div>
